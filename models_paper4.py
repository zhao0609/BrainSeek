...
from transformers import CLIPVisionModelWithProjection, CLIPTextModelWithProjection, CLIPTokenizer, CLIPTextConfig 

from accelerate import Accelerator  # 确保这行存在
# # 初始化加速器
...
class VersatileDiffusionPriorNetwork(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        image_embed = image_embed.view(len(image_embed),-1,1024)
        # text_embed = text_embed.view(len(text_embed),-1,768)
        brain_embed = brain_embed.view(len(brain_embed),-1,1024)
...
class Adapter_Layer(nn.Module):#这是一个适配器层，用于减少和恢复输入维度，具有可选的层归一化和学习率缩放
    def __init__(self,
                 in_channels,
                 bottleneck=32,
                 out_channels=None,
...
class ResMLP(nn.Module):    #一个包含多个残差块的多层感知器（MLP）
...
class MindSingle(nn.Module):    #这是一个用于大脑信号处理和嵌入生成的模型：
...
class FuseTransEncoder(nn.Module):
    def __init__(self,  num_layers, hidden_size, nhead):
        ...
        self.Linear1 = nn.Linear(hidden_size, 768)
        self.hidden_size = hidden_size

    def forward(self, tokens):
        tokens = normalize(tokens, p =2 ,dim =2) 
        encoder_X = self.transformerEncoder(tokens)
        encoder_X_r = encoder_X.reshape( -1,257,self.hidden_size)
        # encoder_X_r = normalize(encoder_X_r, p =2 ,dim =1) 
        x = self.Linear1(encoder_X_r)
        return x
    


class FuseTransAndCrossAttention(nn.Module):
    def __init__(self,  num_layers, hidden_size, nhead):
        super(FuseTransAndCrossAttention, self).__init__()
        encoder_layer = TransformerEncoderLayer(d_model=hidden_size, nhead=nhead, batch_first=False)
        self.transformerEncoder = TransformerEncoder(encoder_layer= encoder_layer, num_layers= num_layers)
        self.crossAttention = CrossAttention(768, 768, 768)
        self.Linear1 = nn.Linear(hidden_size, 768)
        self.hidden_size = hidden_size

    def forward(self, x_1, x_2):
        x_1 = normalize(x_1, p =2 ,dim =2) 
        # x_2 = normalize(x_2, p =2 ,dim =2) 

        x_2 = self.transformerEncoder(x_2)
        # x_2 = normalize(x_2, p =2 ,dim =2) 
        x_2 = self.Linear1(x_2)
        final_fused_features, _ = self.crossAttention(x_2, x_1)
