...
parser.add_argument(
    "--model_name", type=str, default="testing_paper4",
...
outdir = '/data/zhaoyuxiao/MindEye_janus_paper4_1111111111111/train/'
...
if use_image_aug:
    ...
    img_augment = AugmentationSequential(
        ...
        kornia.augmentation.Resize((384, 384)),
...
train_dl, val_dl, num_train, num_val = utils.get_dataloaders(
    ...
    train_url=train_url,#/train/****.tar
    val_url=val_url,#/test/*****.tar
    meta_url=meta_url,# metadata_subj01.json
    num_train=num_train,
    num_val=num_val,
...
if subj == 1:
    ...
    num_voxels = 14386
voxel2clip_kwargs = dict(in_dim=num_voxels,out_dim=576*1024,clip_size=clip_size,use_projector=use_projector)
...
diffusion_prior, optimizer, lr_scheduler = accelerator.prepare(
diffusion_prior, optimizer, lr_scheduler
)
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, coco) in enumerate(train_dl):
        ...
        with torch.cuda.amp.autocast():
            ...
            if use_image_aug:
                image = img_augment(image).to(device)
                # plt.imshow(utils.torch_to_Image(image))
                # plt.show()

            voxel = voxel[:,repeat_index].float().to(device)
            ...
            clip_target = vl_gpt.prepare_inputs_embeds(image.half()) #1*630*4096
            ...
            print(f"epoch:{epoch}, train_i:{train_i}, loss_prior:{loss_prior}, loss_nce:{loss_nce}, loss_prior:{loss_prior}, loss:{loss.item()}, sims_base:{sims_base}")
    ...
    for val_i, (voxel, image, coco) in enumerate(val_dl): 
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                clip_target = vl_gpt.prepare_inputs_embeds(image) #1*630*4096
                ...
                print(f"epoch:{epoch},val_i:{val_i},val_loss_nce:{val_loss_nce}, val_loss_prior:{val_loss_prior},loss:{val_loss.item()}, val_sims_base:{val_sims_base}")
...
if __name__ == '__main__':
    model = CLIPModel.from_pretrained("data/ZhaoYuXiao/UniDiffuser/openai/clip-vit-large-patch14").cuda()
    tokenizer = AutoTokenizer.from_pretrained("data/ZhaoYuXiao/UniDiffuser/openai/clip-vit-large-patch14")
    processor = AutoProcessor.from_pretrained("data/ZhaoYuXiao/UniDiffuser/openai/clip-vit-large-patch14")


    #pormet

    #raw_coco_mat = '/data/WangBoWen/OpenSet/raw/overall/coco2017.mat'  coco数据集

    raw_mat = h5py.File(raw_coco_mat)

    IAll = raw_mat['IAll'][:].transpose(0, 3, 2, 1)
    LAll = raw_mat['LAll'][:].T
    #text = open('/data/WangBoWen/OpenSet/raw/overall/coco.txt', 'r').readlines()
    ...
    with torch.no_grad():
        for i in range(len(text)):
            ...
            txts.append(txt_features.cpu())

            if i % 10000 == 0:
                print(i)

    txts_v = np.vstack(txts)
    del txts
    print(txts_v.shape)

    txts_mean = []
    for i in range(0, txts_v.shape[0], 5):
        txt_mean = np.sum(txts_v[i:i + 5], axis=0) / 5
        txts_mean.append(txt_mean)

    txts_v_mean = np.vstack(txts_mean)
    del txts_mean
    print(txts_v_mean.shape)


    imgs = []
    with torch.no_grad():
        for i in range(IAll.shape[0]):
            im = Image.fromarray(IAll[i]).convert('RGB')
            inputs = processor(images=[im], return_tensors="pt")
            inputs['pixel_values'] = inputs['pixel_values'].cuda()
            image_features = model.get_image_features(**inputs)
            imgs.append(image_features.cpu())
    ...
    # 保存
    data = {'IAll': imgs, 'TAll': txts_v_mean, 'LAll': LAll}
    save_path = '/data/WangBoWen/OpenSet/feature/overall/coco.mat'
    hdf5storage.write(data, '.', save_path, store_python_metadata=False, matlab_compatible=True)